    ...
    PowStrategy: spongefish_pow::PowStrategy,
{
            ...
            + HintSerialize,
    {
        ...
        if round_state.round == self.config.n_rounds() {
            return self.final_round(prover_state, round_state, &folded_evaluations);
        }
        ...
        let leaf_ordering_mode = if self.config.reed_solomon.is_bit_reversed() {
        ...
        let mut answers: Vec<_> = stir_challenges_indexes
            .iter()
            .map(|i| {
                // TODO should this be reed_solomon or basefield?
                let o = if self.config.reed_solomon.is_bit_reversed() {
                    reverse_bits(*i, size)
                } else {
                    *i
                };
